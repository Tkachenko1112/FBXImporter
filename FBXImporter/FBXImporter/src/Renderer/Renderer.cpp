...
glm::mat4 getTransform(Animation& animation, std::string& bonename, float time) {
    ...
    float animationTime = time; // Ensure animation loops
    ...
    for (const Channel& channel : animation.channels) {
        if (channel.boneName == bonename) {
            ...
            glm::quat interpolatedRotation = glm::mix(prevQ, nextQ, t);
            glm::vec3 interpolatedRotation3 = glm::vec3(interpolatedRotation.x, interpolatedRotation.y, interpolatedRotation.z);
...
void Renderer::RenderFrame(float current_time) {
    ...
    for (unsigned int i = 0; i < _skinnedModel._bones.size(); i++) {
        ...
        glm::mat4 trans;
        ofbx::Matrix parentTrans = _skinnedModel._bones[i]->getParent()->getLocalTransform();
        ofbx::Matrix localtrans = _skinnedModel._bones[i]->getLocalTransform();
        ...
        for (int j = 0; j < _skinnedModel._animations.size(); ++j)
        {
            std::string name = _skinnedModel._bones[i]->name;
            trans = getTransform(_skinnedModel._animations[j], name, current_time);
            glm::mat4 modelMatrix = trans * localTransA;
